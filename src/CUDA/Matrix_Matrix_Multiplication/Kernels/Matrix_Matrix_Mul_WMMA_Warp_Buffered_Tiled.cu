...
__device__ __forceinline__ void Load_Tile( const float* A,const float* B,
                                          __half Adata[2][BLOCK_TILE_K][BLOCK_TILE_M +8],
                                          __half Bdata[2][BLOCK_TILE_K][BLOCK_TILE_N +8],
                                           int M, int N, int K,
                                           int i0,int j0,int tid,
                                           int tile,int buffer);

template<typename AFrag, typename BFrag, typename CFrag>
__device__ __forceinline__ void Compute_Tile(__half Adata[2][BLOCK_TILE_K][BLOCK_TILE_M +8],
                                             __half Bdata[2][BLOCK_TILE_K][BLOCK_TILE_N +8],
                                             AFrag &A_fragments,
                                             BFrag &B_fragments,
...
extern "C" __global__ void Matrix_Matrix_Mul_WMMA_Warp_Buffered_Tiled( const float* __restrict__ A,
                                                                      ...
                                                                      int M, int N, int K)
{
    ...
    __shared__ __align__(16) __half Adata[2][BLOCK_TILE_K][BLOCK_TILE_M +8];
    ...
    for (int tile = 0; tile < NUMBER_OF_K_TILES; tile++) {
        ...
        Compute_Tile(Adata, Bdata, A_fragments, B_fragments, Accumulated_fragments,
                     warp_row, warp_col,
                     buffer);
    ...
    Write_Tile(C, Accumulated_fragments, M, N, i0, j0, warp_row, warp_col);
                                           ...
                                           int tile,int buffer)
{
    ...
    const int k_tile = tile * BLOCK_TILE_K;
      ...
      // Load BLOCK_TILE_M×BLOCK_TILE_K slice of A using col offset tile*BLOCK_TILE_K
        ...
        for (int i = 0; i < A_LOADS; i++) {
            ...
            // convert to fp16
            Adata[buffer][s_col][s_row] = (i_row < M && j_col < K) ?
    ...
    // Load BLOCK_TILE_K×BLOCK_TILE_N slice of B using row offset tile*BLOCK_TILE_K
        ...
        for (int i = 0; i < B_LOADS; i++) {
            ...
            Bdata[buffer][s_row][s_col] = (i_row < K && j_col < N) ?
                                             ...
                                             int buffer)
{
    ...
    for (int k = 0; k < NUM_WMMA_TILES_K; k++)
    {
        ...
        {
            const int row_ind = warp_row*WARP_TILE_M + m*WMMA_TILE_M;
            const int k_col_ind = k*WMMA_TILE_K;

            wmma::load_matrix_sync(A_fragments[m], &Adata[buffer][k_col_ind][row_ind], BLOCK_TILE_M + 8);

        }
        #pragma unroll
        for (int n = 0; n < NUM_WMMA_TILES_N; n++)
        {
            const int col_ind = warp_col*WARP_TILE_N + n*WMMA_TILE_N;
            const int k_row_ind = k*WMMA_TILE_K;

            wmma::load_matrix_sync(B_fragments[n], &Bdata[buffer][k_row_ind][col_ind], BLOCK_TILE_N + 8);
