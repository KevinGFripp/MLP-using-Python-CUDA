...
__device__ __forceinline__ void Write_Tile(float* C, float registers[THREAD_TILE_M][THREAD_TILE_N],
                                           int M, int N,
                                           int i0, int j0,
                                           int warp_row, int warp_col,
                                           int thr_row, int thr_col);

extern "C" __global__ void Matrix_Matrix_Mul_Warp_Buffered_Reg_Tiled( const float* __restrict__ A,
                                                                      ...
                                                                      int M, int N, int K)
{
    ...
    // lane = col + (WARP_THREADS_N * row)         (row-major)
    ...
    Write_Tile(C, registers, M, N, i0, j0,
               warp_row, warp_col, thr_row, thr_col);
  ...
  // write back to global memory THREAD_TILE_M*THREAD_TILE_N elements
    ...
    for (int m = 0; m < THREAD_TILE_M; m++)
    {
        ...
        for (int n = 0; n < THREAD_TILE_N; n++)
        {
            ...
            if (row < M && col < N) C[row * N + col] = registers[m][n];
        }
    }
