...
__device__ __forceinline__ void Write_Z_Tile(float* C, const float* b,
                                             float registers[THREAD_TILE_M][THREAD_TILE_N],
                                             int M, int N,
                                             int i0, int j0,
                                             int warp_row, int warp_col,
...
extern "C" __global__ void forward_propagate_kernel( const float* __restrict__ A,
                                                     ...
                                                     int M, int N, int K)
{
    ...
    Write_Z_Tile(C, b, registers, M, N, i0, j0, warp_row, warp_col, thr_row, thr_col);
  ...
  // write back to global memory THREAD_TILE_M*THREAD_TILE_N elements
    ...
    for (int m = 0; m < THREAD_TILE_M; m++)
    {
        ...
        for (int n = 0; n < THREAD_TILE_N; n++)
        {
            ...
            // z = W*a + bias
            if (row < M && col < N) C[row * N + col] = registers[m][n] + b[row];
        }
    }
